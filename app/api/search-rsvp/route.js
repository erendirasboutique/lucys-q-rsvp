  ...
  try {
    ...
    if (!fullName || !phoneNormalized) {
      return NextResponse.json(
        { ok: false, error: "Enter your full name and phone number." },
        { status: 400 }
      );
    }
    ...
    if (!data) {
      return NextResponse.json(
        {
          ok: false,
          error: "We could not find your RSVP. Check the name and phone number.",
        },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { ok: false, error: error.message || "Search error." },
      { status: 500 }
    );
  }
}
